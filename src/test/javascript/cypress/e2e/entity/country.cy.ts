...
describe('Country e2e test', () => {
  ...
  let username: string;
  let password: string;
  const countrySample = { isoCode: 'qua', name: 'never self-confidence salty' };
  ...
  describe('Country page', () => {
    it('should have translated page title', () => {
      ...
      cy.getEntityHeading('Country').should('not.contain', 'proficiencyTestingApp.country.home.title');
    });
  ...
  describe('new Country page', () => {
    ...
    it('should create an instance of Country', () => {
      cy.get(`[data-cy="isoCode"]`).type('per');
      cy.get(`[data-cy="isoCode"]`).should('have.value', 'per');

      cy.get(`[data-cy="name"]`).type('permafrost steak excluding');
      cy.get(`[data-cy="name"]`).should('have.value', 'permafrost steak excluding');

      cy.get(`[data-cy="region"]`).type('indeed twist however');
      cy.get(`[data-cy="region"]`).should('have.value', 'indeed twist however');
