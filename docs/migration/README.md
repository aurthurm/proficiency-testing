...
The source baseline used for this implementation is [`deforay/ept`](https://github.com/deforay/ept) commit `f25f00a0fac4901b146221096870526fed241c68` (baseline schema 7.6.1 plus 43 upgrade scripts through 7.6.20). The baseline `sql/init.sql` contains 105 tables.
...
7. Reconciliation fails the run when row counts, stale rows, typed identity maps, or file checksums differ.
...
Scheme-specific result tables and the remaining legacy configuration/audit tables are deliberately retained in the lossless archive until their typed domain slices are implemented. They are not discarded or flattened into the current generic result model.
...
3. Start with an empty PostgreSQL database and let Liquibase create the target schema.
...
Example environment (supply secrets through the deployment secret manager):
...
export EPT_LEGACY_JDBC_URL='jdbc:mysql://legacy-db:3306/pt?useUnicode=true&characterEncoding=utf8&useSSL=true'
export EPT_LEGACY_DB_USER='migration_reader'
export EPT_LEGACY_DB_PASSWORD='...'

./mvnw -Pprod -DskipTests spring-boot:run
```
...
The source database account only needs read access. The application target account must be allowed to write the application and migration tables. Migration is disabled by default.
...
The application exits with an error when reconciliation fails and `EPT_MIGRATION_FAIL_ON_ERROR=true`.
